...
Function GetConfigPath([String]$configName) {
    return "$scoopTarget\persist\scoopydoo\config\$configName"
}
...
Function EnsurescoopydooVersion($configPath) {
    $scoopConf = (Get-Content "$configPath\conf.json") | ConvertFrom-Json
    if ($scoopConf.scoopydoo -and $scoopConf.scoopydoo.version) {
        LogUpdate "Check scoopydoo version..."
        $( (Get-Item "$PSScriptRoot\..").Target ) -match '(?<version>[^\\]+$)' > $null
        $version = [System.Version]::Parse($scoopConf.scoopydoo.version)
        $current_version = [System.Version]::Parse($matches['version'])
        cleanupscoopydoo $current_version
        if ($version -eq $current_version) {
            LogMessage "scoopydoo $current_version used"
        } else {
            LogMessage "Updating scoopydoo to $version accordingly to the configuration..."
            exec {
                scoop install "scoopydoo/scoopydoo@$version"
            } -catchOutput "Could not install" -exit
            LogInfo "scoopydoo has been updated acordingly to the configuration."
            LogMessage "Re Invoke to use the right scoopydoo version:"
            LogMessage ""
            LogMessage "     $global_command"
            exit
        }
    } else {
        LogWarn "No scoopydoo version specified in the configuration. Aborting..."
        exit
    }
}

# taken from scoop versions.ps1
function cleanupscoopydoo($current_version) {
    $versions = versions "scoopydoo " $global | Where-Object { $_ -ne $current_version -and $_ -ne 'current' }
    if (!$versions) {
        return
    }
    write-host -f yellow "Removing scoopydoo`:" -nonewline
    $versions | ForEach-Object {
        $version = $_
        write-host " $version" -nonewline
        $dir = versiondir "scoopydoo" $version $global
